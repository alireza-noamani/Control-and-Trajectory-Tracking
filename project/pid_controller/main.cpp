...
int main ()
{
  ...
  **/
  ...
  // PID controller for steering is initialized with Kp=0.7, Kd=0.042, Ki=0.4, output_lim_max=1.2, output_lim_min=-1.2
  PID pid_steer = PID();
  ...
  **/
  ...
  // PID controller for steering is initialized with Kp=0.2, Kd=0.04, Ki=0.15, output_lim_max=1.0, output_lim_min=-1.0
...
//           // Update the delta time with the previous command
          ...
          **/
          // First, I found the closest waypoint to the current position of the vehicle. Then, the closest waypoint and the current position of the vehicle was used to determine the desired yaw angle.
          int min_dist_idx = 0;
          for (int i = 0; i < x_points.size(); i++){
            double min_distance = std::hypot((x_position - x_points[min_dist_idx]), (y_position - y_points[min_dist_idx]));
            double current_distance = std::hypot((x_position - x_points[i]), (y_position - y_points[i]));
            if (current_distance < min_distance){
              min_dist_idx = i;
            }
          }
          double desired_yaw = angle_between_points(x_position, y_position, x_points[min_dist_idx], y_points[min_dist_idx]);
          ...
          **/
          ...
          // the throttle error was defined as the difference between the closest waypoint velocity (desired velocity) and current velocity of the vehicle
          double desired_velocity =  v_points[min_dist_idx];
